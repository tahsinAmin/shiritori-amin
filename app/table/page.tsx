...
const FilterableTable = () => {
  const [data] = useState([
    { firstName: 'Jon', status: 'Active', deliveryDate: '2025-10-10', expiryDate: '2025-10-10', fullName: 'Jon Snow' },
    { firstName: 'Cersei', status: 'Inactive', deliveryDate: '2025-10-10', expiryDate: '2025-10-10', fullName: 'Cersei Lannister' },
    { firstName: 'Jaime', status: 'Pending', deliveryDate: '2025-10-10', expiryDate: '2025-10-10', fullName: 'Jaime Lannister' },
    { firstName: 'Arya', status: 'Active', deliveryDate: '2025-10-10', expiryDate: '2025-10-10', fullName: 'Arya Stark' },
    { firstName: 'Daenerys', status: 'Inactive', deliveryDate: '2025-10-10', expiryDate: '2025-10-10', fullName: 'Daenerys Targaryen' }
  ]);
  const [filterList, setFilterList] = useState({ expire_in: '', expired: '', status: '', firstname: '' });
  ...
  const columns = [
    ...
    { key: 'deliveryDate', label: 'Delivery Date', filterable: true },
    { key: 'expiryDate', label: 'Expiry Date', filterable: true },
  ...
  const handleValueChange = (operator: string, field: string, value: string) => {
    console.log('activeFilter', activeFilter);
    console.log('operator', operator);
    console.log('field', field);
    console.log('value', value);

    if (activeFilter === 'expiryDate' && operator === 'within' && field === 'value2') {
      setFilterList(prev => ({ ...prev, expired: '', expire_in: `${filters[activeFilter]['value']},${value}` }))
    }
    if (activeFilter === 'expiryDate' && operator === 'before') {
      setFilterList(prev => ({ ...prev, expire_in: value, expired: '' }))
    }
    if (activeFilter === 'expiryDate' && operator === 'equals') {
      setFilterList(prev => ({ ...prev, expired: value, expire_in: '' }))
    }
  ...
  const renderValueInput = () => {
    const currentFilter = filters[activeFilter as string] || { operator: 'equals', value: '', value2: '' };
    ...
    if (currentFilter.operator === 'equals' || currentFilter.operator === 'sameAs') {
      return (
        <select
          ...
          onChange={(e) => handleValueChange(currentFilter.operator, 'value', e.target.value)}
        >
        ...
        <div className="space-y-2">
          ...
          {currentFilter.value !== '' ?
            (<input
              ...
              onChange={(e) => handleValueChange(currentFilter.operator, 'value2', e.target.value)}
    ...
    if (activeFilter === 'fullName') {
      return (
        <>
          ...
          <option value="sameAs">Same as First and Status</option>
        </>
      );
    }
    return (
